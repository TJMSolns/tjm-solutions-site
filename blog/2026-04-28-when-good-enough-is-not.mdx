---
title: "When Good Enough is Not"
description: "Why commerce systems that read fine to a human buyer routinely fail to give a purchasing agent a decision it can safely act on -- and how algebraic data types close that gap."
date: 2026-04-28
authors: [tony]
...
<head>
  <link rel="canonical" href="https://tmoores.medium.com/when-good-enough-is-not-9b1287979f49" />
  <meta property="og:type" content="article" />
  <meta property="article:published_time" content="2026-04-28T00:00:00Z" />
  <meta property="article:author" content="Tony Moores" />
  <script type="application/ld+json">{`{"@context":"https://schema.org","@type":"BlogPosting","headline":"When Good Enough is Not","description":"Why commerce systems that read fine to a human buyer routinely fail to give a purchasing agent a decision it can safely act on -- and how algebraic data types close that gap.","datePublished":"2026-04-28","author":{"@type":"Person","name":"Tony Moores","url":"https://www.tjm.solutions/about"},"publisher":{"@type":"Organization","name":"TJM Solutions","url":"https://www.tjm.solutions"},"url":"https://www.tjm.solutions/articles/2026/04/28/when-good-enough-is-not","mainEntityOfPage":{"@type":"WebPage","@id":"https://www.tjm.solutions/articles/2026/04/28/when-good-enough-is-not"}}`}</script>
</head>
...
```scala
final case class ProductAvailability(
 sku: String,
 status: String,
 message: String,
 quantityAvailable: Option[Int],
 estimatedShipDate: Option[String]
)
```
...
```scala
def nextStep(result: AvailabilityResult): BuyerAction =
  result match
    case AvailabilityResult.Available(quantity, window) =>
      BuyerAction.Reserve(quantity, window)

    case AvailabilityResult.Backordered(expectedDate) =>
      BuyerAction.CompareAlternatives(expectedDate)

    case AvailabilityResult.Restricted(reason) =>
      BuyerAction.Escalate(reason)

    case AvailabilityResult.Discontinued(replacement) =>
      BuyerAction.EvaluateReplacement(replacement)

    case AvailabilityResult.Unknown(reason) =>
      BuyerAction.RetryOrEscalate(reason)
```
...
```scala
final case class PriceResponse(
  sku: String,
  price: BigDecimal,
  currency: String,
...
```scala
def decideAvailability(
  inventory: InventorySnapshot,
  request: AvailabilityRequest
): AvailabilityResult =
  inventory.find(request.sku) match
    case Some(item) if item.available >= request.quantity =>
      AvailabilityResult.Available(
        quantity = item.available,
        reservationWindow = item.reservationWindow
      )

    case Some(item) if item.backorderDate.isDefined =>
      AvailabilityResult.Backordered(
        expectedDate = item.backorderDate
      )

    case Some(item) if item.restricted =>
      AvailabilityResult.Restricted(
        reason = RestrictionReason.BuyerNotEligible
      )

    case Some(item) if item.discontinued =>
      AvailabilityResult.Discontinued(
        replacement = item.replacement
      )

    case Some(_) =>
      AvailabilityResult.Unknown(
        reason = AvailabilityUnknownReason.InconsistentInventoryState
      )

    case None =>
      AvailabilityResult.Unknown(
        reason = AvailabilityUnknownReason.SkuNotFound
      )
```
...
```scala
final case class AvailabilityDecisionRecord(
  request: AvailabilityRequest,
  result: AvailabilityResult,
  factsUsed: List[FactReference],
  decidedAt: Instant
)
```
