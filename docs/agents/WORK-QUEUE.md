...
|----|------|-------|--------|-----------|
| WQ-027 | **STANDING RULE + AUDIT** — Medium-parity invariant (Tony directive 2026-06-10, org HL-068): `https://www.tjm.solutions/articles` must be a **superset** of `https://tmoores.medium.com/` — every Medium article must exist on the site; site-only articles are fine (the reverse is not required). Two parts: (a) **codify** the rule in site rails (CLAUDE.md + CONTEXT-KERNEL quality gates, alongside the POL-002 deploy gates from WQ-020) so every future site session checks it; (b) **run the initial parity audit** — fetch the Medium article index, diff against `blog/*.mdx`, and queue one WQ item per missing article (apply the established JSON-LD + canonical-link + tag-taxonomy pattern). Cross-ref: recent-window diff done 2026-06-10 (profile page, truncated) — found Parts 12/13/14/15 missing, now covered by WQ-021/WQ-028/WQ-024/WQ-029; remaining audit work is the full index sweep (older/truncated entries) to confirm nothing else is missing. | Claude | Queued | — |
...
- **Ordering convention (GL-017/GL-018, updated 2026-06-10):** Claude-unblocked (WQ-027/018/019/020/021/024/028/029/002) → blocked-on-Medium-publication (WQ-022; Tony action) → blocked-on-revenue-chain (WQ-016/017) → hard-blocked on org gate (WQ-005–007; WQ-003).
